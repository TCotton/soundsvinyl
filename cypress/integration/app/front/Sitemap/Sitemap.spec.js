...
describe('Trying a navigation test', () => {
	it ('is should visit the site', () => {
...
describe("Sitemap", () => {
  ...
  it("lists all tags and pages in correct order", () => {
		cy.fixture('dataGetAll.json').as('dataGetAll');
		cy.fixture('dataGetTags.json').as('dataGetTags');
		cy.server();
		cy.route('GET', `https://localhost:3000/apiV1/page/getall`, '@dataGetAll');
		cy.route('GET', `https://localhost:3000/apiV1/page/getTags`, '@dataGetTags');
		cy.visit('https://localhost:3000/sitemap');
		cy.contains('Sitemap');
		cy.contains('rock');
		cy.contains('Primal Scream - Carry Me Home');
  });
});
